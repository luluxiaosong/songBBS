...
class Home extends base_Controller
{
    ...
    */
    public function index()
    {
        ...
        //热门帖子 10条 按评论数排
        $this->db->cache_off(); // 按时间查询 这里不能缓存
    ...
    //按话题 展示示帖子
    public function posts_by_topic()
    {
        $this->db->cache_on();
        ...
        if ($data['option'] == 'new') {
            $param = [
                ...
                'per_page' => $this->config->item('page_nums'),//每页条数
                'url' => site_url("home/posts_by_topic/$topic_id?option=new"),
                'page' => $page //当前页数
            ];
            $this->load->library('page_turn', $param);
        ...
        $this->db->cache_on();
        $data['topics_all'] = $this->topic_m->topics_all();
        $this->load->view('home/posts_by_topic', $data);
    }
